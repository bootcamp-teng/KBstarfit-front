<template>
    <div class="container">
        <div v-if="listBoxType === 1" style="padding-left: 120px;">
            <select size="10" name="" id="" class="form-control" style="height: 200px; width: 100px;">
                <option class="option-text m-text" :key="i" :value="d" @click="selected(d)" v-for="(d, i) in day">{{ d }}</option>
            </select>
        </div>
...
//   import $ from "jquery";
  ...
  export default {
    ...
    methods: {
        ...
        selected(d) {
            if (this.GE_LIST_BOX_TYPE === 1) this.AC_SET_DAY(d);
